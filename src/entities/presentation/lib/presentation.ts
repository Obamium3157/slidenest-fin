...
export function addText(
  ...
  const textObj: SlideText = { type: "text", id, text, font, rect };
...
// выкинуть Partial
// в тестах проверить, можно ли добавить несуществующие поля
function updateSlideObj<T extends SlideObj>(
  ...
  const slide: Slide | undefined = getOrderedMapElementById(
    presentation.slides,
    slideId,
  );
  if (!slide) {
  ...
  const slideObj: SlideObj | undefined = getOrderedMapElementById(
    slide.slideObjects,
    objId,
  );
  if (!slideObj) {
  ...
  const newObj = { ...slideObj, ...updates } as T;

  const newSlide: Slide = {
    ...slide,
    slideObjects: getNewOrderedMapWithPushed(slide.slideObjects, objId, newObj),
  };
